...
window.onload = function(){
	...
	var aLi = oBox.getElementsByTagName('div');
	...
	for(i=0;i<aLi.length;i++){
		...
		aLi[i].onclick = function(){
			var imgurl=aImg[this.index].getAttribute("src");
			...
			oFrame.innerHTML = '<img src='+imgurl+'>';
	...
	aBli[0].onclick = function(){
		...
		var imgurl=aImg[iNow].getAttribute("src");
		oFrame.innerHTML = "<img src="+imgurl +" style='width:100%;height:100%'/>";
	...
	aBli[2].onclick = function(){
		...
		var imgurl=aImg[iNow].getAttribute("src");
		oFrame.innerHTML = oFrame.innerHTML = "<img src="+imgurl + " style:'width=100%; height=100%;' />";
	};
	aBli[3].onclick = function(){
		startMove(oFrame, {opacity:0, left:aImg[iNow].offsetLeft, top:aImg[iNow].offsetTop, width:150, height:100}, function(){
